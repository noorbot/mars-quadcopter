...
def ignore_store_ttb_points(outlier_cloud_store):
    ...
    ttb3_x = trans_ttb3[0]
    ttb3_y = trans_ttb3[1]
    center3 = np.array([ttb3_x, ttb3_y, 0.1])
    radius3 = 0.4
    distances3 = np.linalg.norm(points_outlier_store - center3, axis=1)
    points_outlier_store = points_outlier_store[distances3 >= radius3]
...
def clean_pointcloud_and_ttbs(outlier_cloud):
    ...
    if len(np.unique(labels)) > -1:  # if robot is on floor the camera will detect zero clusters and the below code should not be executed
        ...
        for cluster in range(max_label+1):
            ...
            if(len(curr_label) < 100): # remove points that are in a cluster with fewer than 100 members 
