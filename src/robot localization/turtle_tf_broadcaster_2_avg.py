...
def turtle_tf_broadcaster():
    ...
    # initialize node
    rospy.init_node('turtle_tf_broadcaster_2')
    ...
    while not rospy.is_shutdown():
        ...
        if(conditionMet == True):       # if we DO have enough aruco readings
            ...
            if do_once == True:
                ...
                rospy.set_param('/robot_3/map_merge/init_pose_x', ttbPose.pose.position.x.item()) # the .item() is needed to convert from numpy float to native python gloat to be accepted
                rospy.set_param('/robot_3/map_merge/init_pose_y', ttbPose.pose.position.y.item())
                rospy.set_param('/robot_3/map_merge/init_pose_z', 0.0)
                rospy.set_param('/robot_3/map_merge/init_pose_yaw' , 0.0) #aruco_yaw)   Set yaw to zero always, as with simulation. Taken care of already by broadcaster
                rospy.loginfo("PARAMS SET robot 3: x:" + str(ttbPose.pose.position.x) + "  y:" + str(ttbPose.pose.position.y) + "  z:" + str(ttbPose.pose.position.z) + "  yaw: 0.0")
