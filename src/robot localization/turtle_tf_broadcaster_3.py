...
def turtle_tf_broadcaster():
    ...
    while not rospy.is_shutdown():
        ...
        if aruco_found == True and do_once == True:
            rospy.set_param('/robot_1/map_merge/init_pose_x', arucoPose.pose.position.x)
            rospy.set_param('/robot_1/map_merge/init_pose_y', arucoPose.pose.position.y)
            rospy.set_param('/robot_1/map_merge/init_pose_z', arucoPose.pose.position.z)
            rospy.set_param('/robot_1/map_merge/init_pose_yaw' , 0.0) #aruco_yaw)   Set yaw to zero always, as with simulation. Taken care of already 
            rospy.loginfo("PARAMS SET robot 1: x:" + str(arucoPose.pose.position.x) + "  y:" + str(arucoPose.pose.position.y) + "  z:" + str(arucoPose.pose.position.z) + "  yaw: 0.0")
            do_once = False

        # publish transform from map to robot_1/map
