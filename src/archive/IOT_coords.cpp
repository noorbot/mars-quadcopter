...
int main(int argc, char **argv)
{
    ...
    pose7.pose.position.z = 1;

    geometry_msgs::PoseStamped pose8;
    pose8.pose.position.x = 0;
    pose8.pose.position.y = 0;
    pose8.pose.position.z = 0;
    ...
    while(ros::ok()){
        ...
        if(current_state.mode == "OFFBOARD" && current_state.armed && order == 1) {
            ...
            pose = pose7;
            if(ros::Time::now() - fly_time > ros::Duration(42.0)) { 
                order++;
