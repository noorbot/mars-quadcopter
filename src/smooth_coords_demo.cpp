...
{
    ros::init(argc, argv, "smooth_coords_demo");
    ros::NodeHandle nh;
    ...
    while(ros::ok()){
        ...
        if(current_state.mode == "OFFBOARD" && current_state.armed && order == 1) {
            ...
            if(ros::Time::now() - fly_time > ros::Duration(13.0+numPoses)) {  // adjust number of seconds here to be as many rows in poseArray
                order++;
