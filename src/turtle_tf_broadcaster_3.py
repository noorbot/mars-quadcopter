...
def turtle_tf_broadcaster():
    ...
    while not rospy.is_shutdown():
        ...
        if aruco_found == True and do_once == True:
            ...
            rospy.set_param('/robot_1/map_merge/init_pose_yaw' , yaw)
            rospy.loginfo("PARAMS SET robot 1: " + str(arucoPose.pose.position.x) + " " + str(arucoPose.pose.position.y) + " " + str(arucoPose.pose.position.z) + " " + str(yaw))
            do_once = False
